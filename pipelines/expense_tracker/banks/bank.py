...
class Bank(ABC):
    ...
    def __init__(self, source: BankSource) -> None:
        ...
        self.account_data[self.BUDGET_MAP_TABLE] = self.transaction_map.drop(
            columns=['Label', 'StringMatch']
        ).drop_duplicates()
    ...
    def _validate_bank(self) -> None:
        ...
        for account_name, files in self.source.accounts.items():
            if not files:
                raise ValueError(
                    f'{self.name}/{account_name} does not contain any transaction CSVs.'
                )
    ...
    def _map_categories(self) -> None:
        ...
        for account_name, account_data in self.account_data.items():
            ...
            map_columns = [
                col
                for col in list(self.transaction_map.columns)
                if col not in ['Label', 'CategorySK']
            ]
    ...
    def _parse_transactions(self) -> dict[str, pd.DataFrame]:
        raise NotImplementedError(
            'Children of "Bank" must implement _parse_transactions(self) -> dict[str,pd.DataFrame]'
        )
